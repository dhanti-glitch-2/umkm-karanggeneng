    ...
    <>
      ...
      <section className="bg-noise relative overflow-hidden bg-teak-dark py-16 text-cream">
        <div className="section-pad relative mx-auto max-w-content">
          <Link href="/umkm" className="font-mono text-xs uppercase tracking-widest text-gold-light">
            ← Kembali ke UMKM
          </Link>
          <p className="eyebrow mt-4 text-gold-light">{umkm.jenisUsaha}</p>
          <h1 className="mt-2 font-display text-3xl font-semibold md:text-5xl">{umkm.nama}</h1>
          <p className="mt-3 font-body text-cream/80">{umkm.tagline}</p>
        </div>
      </section>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {umkm.galeri.map((foto, i) => (
                <div key={i} className="card-wood relative h-56 w-full overflow-hidden">
                  <Image src={foto} alt={`${umkm.nama} ${i + 1}`} fill className="object-cover" />
                </div>
              ))}
            </div>
            ...
            {umkm.menu && umkm.menu.length > 0 && (
              <div className="mt-10">
                ...
                <div className="mt-4 space-y-6">
                  {umkm.menu.map((grup) => (
                    <div key={grup.judul}>
                      <h3 className="font-display text-lg text-ink">{grup.judul}</h3>
                      <ul className="mt-2 divide-y divide-teak/10">
                        {grup.items.map((item) => (
                          ...
                          >
                            <span className="text-ink/80">{item.nama}</span>
                            <span className="font-mono text-sawah-dark">{item.harga}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
            ...
            <div className="card-wood p-6">
              ...
              <dl className="mt-4 space-y-3 font-body text-sm">
                ...
                <div>
                  ...
                  <dd className="text-ink/90">{umkm.jamOperasional}</dd>
                </div>
                {umkm.pirt && (
                  <div>
                ...
                <div>
                  ...
                  <dd className="text-ink/90">{umkm.pembayaran.pembayaran.join(", ")}</dd>
                </div>
                <div>
                  <dt className="text-ink/50">Cara Pemesanan</dt>
                  <dd className="text-ink/90">{umkm.pembayaran.pemesanan.join(", ")}</dd>
                </div>
              </dl>
